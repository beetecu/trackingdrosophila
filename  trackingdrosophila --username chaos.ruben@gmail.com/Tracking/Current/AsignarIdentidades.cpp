...
void MotionTemplate(tlcde* framesBuf, tlcde* Etiquetas) {
	...
	if (CREATE_TRACKBARS) {
		...
		cvCreateTrackbar("MHI Duration", "Motion", &TBposMHI, 500);
		cvCreateTrackbar("MAX TIME DELTA", "Motion", &TBposDMax, 100);
		cvCreateTrackbar("MIN_TIME_DELTA", "Motion", &TBposDMin, 50);
	...
	for (i = -1; i < seq->total; i++) {
		...
		if (i < 0) { // case of the whole image
			comp_rect = cvRect(0, 0, cvSize(mhi->width, mhi->height).width,
					cvSize(mhi->width, mhi->height).height);
...
void corregirEstado(STFrame* frame0, STFrame* frame1, STFrame* frame2, int pos) {
	...
	if (fly2) {
		...
		fly0 = (STFly*) obtener(frame0->Flies->numeroDeElementos - 1,
				frame0->Flies);
		// corregimos el numero de frame
...
int buscarFlies(STFrame* frameData, CvRect MotionRoi, int *p) {
	...
	for (i = 0; i < flies->numeroDeElementos; i++) {
		...
		if (fly->Estado == 0)
			continue; // si no está en el fg continuamos
		// si su centro no está dentro de la motionRoi continuamos
		if ((fly->posicion.x < MotionRoi.x) || (fly->posicion.x > (MotionRoi.x
				+ MotionRoi.width)))
			continue;
		if ((fly->posicion.y < MotionRoi.y) || (fly->posicion.y > (MotionRoi.y
				+ MotionRoi.height)))
			continue;
